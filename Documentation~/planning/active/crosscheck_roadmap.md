...
| 4 | Hydrology | §5 | **NEXT** | `hydrology_cross_reference.md` (planned) | — |
...
**Hydrology is next** because:
- Phase L (water systems) is the next major pipeline phase after M/M2
- The technique reports (2a/2b) cover rivers, drainage, flow accumulation, lake detection
- All six game reports have water system documentation (DF strongest, MC moderate, RW moderate)
- Erosion simulation (§3 NONE item, top-12 priority) depends on hydrology design decisions
- Phase M's moisture field optionally consumes FlowAccumulation from Phase L

**Placement & Traversal** follows hydrology. Pipeline Infrastructure is architectural
rather than technique-based and does not benefit from game × technique cross-checking.
